...
class ImportGOCC2bq(Task.Task):
    ...
    #SQL2UNIMA_GOODS = 'SELECT SAFE_CAST(GID AS string) AS GID, SAFE_CAST(AVAILABILITY AS string) AS AVAILABILITY, SAFE_CAST(SALE_PRICE AS string) AS SALE_PRICE, SAFE_CAST(UPDATE_TIME as DATETIME) AS UPDATE_TIME,  * EXCEPT (GID, AVAILABILITY, SALE_PRICE, UPDATE_TIME) FROM {}.{}'
    SQL2UNIMA_GOODS = 'SELECT  SAFE_CAST(gid AS string) AS gid,  SAFE_CAST(availability AS string) AS availability,  substr(string(update_time, "UTC"),1,19) as update_time,  * EXCEPT (gid,availability,update_time) from {}.{}'
    ...
    SQL2UNIMA_GOODSCATECODE = 'SELECT SAFE_CAST(GID AS string) AS GID, SAFE_CAST(CATEGORY_CODE AS string) AS CATEGORY_CODE, SAFE_CAST(LE AS string) AS LE, SAFE_CAST(INSERT_DATE AS DATETIME) AS INSERT_DATE, SAFE_CAST(UPDATE_TIME AS DATETIME) AS UPDATE_TIME,  * EXCEPT (GID, CATEGORY_CODE, LE, INSERT_DATE,  UPDATE_TIME) FROM {}.{}'
